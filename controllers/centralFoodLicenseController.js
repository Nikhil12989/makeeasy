...
export const createCentralFoodLicense = async (req, res) => {
    try {
        ...
        console.error("Error creating Central Food License:", error);
        res.status(500).json({ message: "Error creating Central Food License", error: error.message });
    }
};
...
export const getAllCentralFoodLicenses = async (req, res) => {
    try {
        ...
        console.error("Error retrieving Central Food Licenses:", error);
        res.status(500).json({ message: "Error retrieving Central Food Licenses", error: error.message });
    }
};
...
export const getCentralFoodLicenseById = async (req, res) => {
    try {
        const { id } = req.params;
        const license = await CentralFoodLicense.findById(id);
        ...
        console.error("Error retrieving Central Food License:", error);
        res.status(500).json({ message: "Error retrieving Central Food License", error: error.message });
    }
};
